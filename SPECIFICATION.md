...
- **Phase 1**: 基盤構築（スレッド管理、データ取得）
- **Phase 2**: UI/UX強化（Excelスタイルのフィルタリング、ソート、ページネーション）
- **Phase 3**: 要約機能（ChatGPT APIによるトピック要約）
- **Phase 4**: 新規スレッド発見（メンション・キーワード検出）
- **Phase 5**: 質問応答機能（Claude Agent SDKによるローカルLLM横断検索）
- **Phase 6**: スレッド専用LLM質問機能（個別スレッド詳細での質問応答）
...
2. **高度なフィルタリング・ソート**
   - Excelライクなテーブルヘッダーフィルタ
   - 検索ボックス（全文検索）
   - 日付範囲フィルタ
   - ページネーション（10/20/50/100件）

3. **AI機能**
   - ChatGPT APIによる要約自動生成
   - Claude Agent SDKによる横断検索
   - 動的な確信度計算
...
- **フィルタリング機能**:
  - **コンパクトフィルタパネル**:
    ...
    - 日付範囲フィルタ（開始日〜終了日）
    - クリアボタン
  - **テーブルヘッダー内ドロップダウンフィルタ**:
    - ステータスフィルタ（すべて/未読/既読・ラジオボタン）
    - タグフィルタ（マルチセレクト・チェックボックス）
- **ソート機能**:
  - カラムヘッダークリックで昇順/降順切り替え
  - 対応カラム: タイトル、メッセージ数、最終更新日時
  - ソート方向インジケーター表示
- **ページネーション**:
  - 表示件数選択（10/20/50/100件）
  - ページ番号ナビゲーション（省略記号付き）
  - 件数表示（例: 1-20 / 45 件）
- **その他機能**:
  - 全スレッド同期ボタン
...
#### 2.5.4 スレッド専用質問機能
- **機能概要**:
  - スレッド詳細画面から特定のスレッドについて質問
  - Claude Agent SDKを使用してローカルで処理
  - 当該スレッドのみを参照して回答（横断検索とは異なる）
...
- **処理フロー**:
  1. スレッドIDと質問をClaude Agent SDKに送信
  2. エージェントが指定されたスレッドのデータのみを参照
  3. スレッド内の情報から回答を生成
  4. 確信度を計算して返却
...
- **UI表示**:
  - スレッド詳細画面の要約セクション直下に配置
  - 複数行テキストエリアでの質問入力
  - キーボードショートカット対応（Ctrl+Enter）
  - 回答は確信度バッジ付きで表示
...
| メソッド | エンドポイント | 説明 |
|---------|--------------|------|
| POST | /api/search/query | 自然言語質問（横断検索） |
| POST | /api/threads/{thread_id}/query | 特定スレッド専用質問 |
...
#### 7.2.5 POST /api/threads/{thread_id}/query

**概要**: 特定スレッド専用の自然言語質問

**パスパラメータ**:
- `thread_id`: スレッドID (例: thread_001)

**リクエストボディ**:
```json
{
  "query": "このスレッドの主な議論ポイントは?"
}
```
...
```json
{
  "answer": "このスレッドでは以下の3つが主な議論ポイントでした:\n\n1. **認証方式の選定**: OAuth 2.0とJWT認証の比較検討が行われ、最終的にOAuth 2.0を採用することで合意\n\n2. **セキュリティ要件**: トークンの有効期限やリフレッシュトークンの扱いについて議論\n\n3. **実装スケジュール**: 2週間での実装を目指し、タスク分担を決定",
  "confidence": 0.85
}
```
...
```json
{
  "detail": "Query processing failed: [エラーメッセージ]"
}
```
(ステータスコード: 500)
...
**使用例**:
```bash
curl -X POST "http://localhost:8000/api/threads/thread_001/query" \
  -H "Content-Type: application/json" \
  -d '{"query": "このスレッドの主な議論ポイントは?"}'
```
...
- **結果サマリー**:
  - 検索結果件数表示（例: 45 件のスレッドが見つかりました）

- **スレッド一覧テーブル**:
  - カラム:
    - タイトル (ソート可能、リンク付き) + 新規バッジ
    - 要約（最大幅300px、省略表示）
    - メッセージ数 (ソート可能、中央揃え)
    - 最終更新 (ソート可能、相対時間表示)
    - ステータス (ヘッダーにドロップダウンフィルタ、バッジ表示)
    - タグ (ヘッダーにドロップダウンフィルタ、チップ表示)
    - アクション (Slackで開くボタン)
  - **Excelスタイルのフィルタ**:
    - ステータス列: ▼ボタンでドロップダウン展開
      - すべて/未読/既読のラジオボタン
    - タグ列: ▼ボタンでドロップダウン展開
      - 複数選択可能なチェックボックス
    - ドロップダウンは外側クリックで自動的に閉じる
  - ソート機能 (カラムヘッダークリック、▲▼で方向表示)
  - 未読スレッドは黄色背景でハイライト

- **ページネーション**:
  - ページ番号ナビゲーション（省略記号付き）
  - 表示件数セレクター (10/20/50/100件)
  - 現在の表示範囲（例: 1-20 / 45 件）
...
- **LLM質問セクション** ⭐ New (Phase 6):
  - セクションタイトル: "このスレッドについて質問"
  - 質問入力フォーム:
    - 複数行テキストエリア (3行)
    - プレースホルダーに使用例を表示
    - キーボードショートカット: Ctrl+Enter で送信
    - 送信ボタン（回答生成中は "回答生成中..." と表示）
  - 回答表示エリア:
    - 回答ラベル（"回答:"）
    - 確信度バッジ（高・中・低を色分け表示）
    - 回答テキスト（改行対応）
...
**状態遷移**:
1. 初期状態: 質問入力エリアとヒントメッセージを表示
2. 質問送信後: ボタンが "回答生成中..." に変わり、入力欄が無効化
3. 回答受信後: 回答とバッジを表示、入力欄を再有効化
4. エラー時: エラーメッセージを赤背景で表示
...
**成果物**:
- ✅ 直感的に操作できる高機能UI
- ✅ Excelライクな操作感でフィルタリング・ソート可能
- ✅ 大量のスレッドでも快適に閲覧可能
- 注: 定期同期機能はPhase 6に移動

---

### Phase 3: 要約機能 (2-3週間) ✅ 完了
**目標**: スレッド情報の整理と可視化
...
### Phase 5: 質問応答機能 (2-3週間) ✅ 完了
**目標**: スレッドを跨いだ情報検索

- Claude Agent SDK統合
  - ✅ SDK初期化（ローカルClaude Code使用）
  - ✅ MCPツール定義（6種類のツール実装）
    - ✅ read_thread_info: スレッド基本情報取得
    - ✅ read_messages: メッセージ取得
    - ✅ read_summary: 要約取得
    - ✅ search_threads: スレッド検索
    - ✅ list_all_threads: 全スレッド一覧取得
...
スレッド一覧画面のフィルタリング条件をビューとして保存・管理できる機能です。ユーザーは頻繁に使用するフィルタ条件を名前付きビューとして保存し、プルダウンから選択することで瞬時に表示条件を切り替えることができます。
...
interface ThreadView {
  id: string;                    // ビューID (view_001)
  name: string;                  // ビュー名 (例: "優先度高タグのスレッド")
  description?: string;          // ビューの説明 (任意)
  is_default: boolean;           // デフォルトビューかどうか
  filters: {
    tags?: string[];             // タグフィルタ (複数選択)
    is_read?: boolean | null;    // 既読/未読フィルタ (true: 既読, false: 未読, null: すべて)
    search?: string;             // 検索キーワード
    date_from?: string;          // 開始日 (ISO 8601形式)
    date_to?: string;            // 終了日 (ISO 8601形式)
    has_new_messages?: boolean;  // 新着メッセージがあるもののみ
  };
  sort?: {
    sort_by?: string;            // ソート項目 (title, message_count, updated_at, created_at)
...
5. **デフォルトビュー設定**
   - ビューをデフォルトとして設定/解除
   - デフォルトビューはアプリ起動時に自動適用

#### 13.2.3 ビュー適用

1. **プルダウンからの選択**
   - スレッド一覧画面のフィルタパネル上部にビュー選択プルダウンを配置
   - プルダウンには以下を表示:
     - 「すべてのスレッド」（フィルタなし）
     - 保存済みビュー一覧
     - 「新規ビュー作成」オプション
...
```
data/
└── views.json              # ビュー定義ファイル
```

#### 13.4.2 views.json フォーマット
  ...
  "views": [
    {
      "id": "view_001",
      "name": "優先度高タグのスレッド",
      "description": "優先度が高いタグが付いているスレッドのみを表示",
      "is_default": true,
      "filters": {
        "tags": ["優先度高"],
        "is_read": null,
...
| GET | /api/views/{view_id} | 個別ビュー取得 |
| POST | /api/views | ビュー作成 |
| PUT | /api/views/{view_id} | ビュー更新 |
| DELETE | /api/views/{view_id} | ビュー削除 |
| PUT | /api/views/{view_id}/default | デフォルトビュー設定 |
...
1. **データモデル** (`backend/models/view.py`)
   - `ThreadView` モデル定義
   - Pydantic によるバリデーション

2. **リポジトリ層** (`backend/repositories/view_repository.py`)
   - `views.json` の読み書き
   - ビューのCRUD操作
   - デフォルトビュー管理

3. **API層** (`backend/api/views.py`)
   - FastAPI エンドポイント実装
   - リクエスト/レスポンス処理
...
1. **型定義** (`frontend/src/types/index.ts`)
   - `ThreadView` インターフェース定義
   - フィルタ条件の型定義

2. **API クライアント** (`frontend/src/lib/api.ts`)
   - ビューCRUD用のAPI呼び出し関数

3. **コンポーネント**
   - `ViewSelector.tsx`: ビュー選択プルダウン
   - `ViewManagementModal.tsx`: ビュー管理モーダル
   - `ViewFormModal.tsx`: ビュー作成/編集モーダル

4. **状態管理**
   - React Query でビュー一覧をキャッシュ
   - 選択中のビューID を state で管理
   - フィルタ条件の変更を検出

5. **統合** (`frontend/src/pages/ThreadListPage.tsx`)
   - ビュー選択プルダウンをフィルタパネル上部に追加
   - ビュー選択時にフィルタ条件を適用
   - 「管理」ボタンでビュー管理モーダルを表示

### 13.7 ユースケース例

#### ユースケース1: 頻繁に使うフィルタをビューとして保存

1. ユーザーがタグフィルタで「優先度高」を選択
2. ステータスフィルタで「未読」を選択
3. ソートを「最終更新日時（降順）」に設定
4. ビュー選択プルダウンから「現在の条件で新規ビュー作成」を選択
5. ビュー作成モーダルが開く
6. ビュー名「優先度高・未読」を入力
7. 「デフォルトビューとして設定」にチェック
8. 「保存」をクリック
9. ビューが保存され、プルダウンに表示される

#### ユースケース2: 保存したビューを適用

1. ユーザーがスレッド一覧画面を表示
2. デフォルトビュー「優先度高・未読」が自動適用される
3. ビュー選択プルダウンから「未読のみ」を選択
4. フィルタ条件が「未読のみ」に即座に切り替わる
5. スレッド一覧が更新される
...
1. ユーザーが「管理」ボタンをクリック
2. ビュー管理モーダルが開く
3. 「優先度高・未読」の「編集」をクリック
4. ビュー編集モーダルが開く
5. 説明を追加: 「優先度が高く、まだ確認していないスレッド」
6. 「保存」をクリック
7. ビューが更新される
...
1. ユーザーが「管理」ボタンをクリック
2. ビュー管理モーダルが開く
3. 不要なビュー「今週更新されたスレッド」の「削除」をクリック
4. 確認ダイアログが表示: 「このビューを削除してもよろしいですか?」
5. 「はい」をクリック
6. ビューが削除される
...
- デフォルトビューは1つのみ設定可能
- 新しいビューをデフォルトに設定すると、既存のデフォルトビューは自動的に解除
- デフォルトビューが削除された場合、次回起動時は「すべてのスレッド」を表示

#### 13.9.2 フィルタ条件の整合性

- ビュー保存時に、空のフィルタ条件は null として保存
- ビュー適用時に、null の条件はフィルタを適用しない
- タグが削除された場合でも、ビューの整合性を維持

#### 13.9.3 パフォーマンス

- ビュー一覧は React Query でキャッシュ
- ビュー適用時は既存のフィルタリングロジックを再利用
- ビュー数が増加してもパフォーマンスに影響しないよう設計

#### 13.9.4 データ移行

- 既存のアプリケーションに追加する場合、`views.json` が存在しない場合は空配列で初期化
- 後方互換性を維持
...
- Phase 6 完了: スレッド専用LLM質問機能実装
  - 特定スレッドのみを参照する質問応答機能
  - スレッド詳細画面にLLM質問セクションを追加
  - 新規APIエンドポイント: `POST /api/threads/{thread_id}/query`
  - 確信度計算アルゴリズムの実装（スレッド専用版）
  - UI/UX設計: キーボードショートカット、確信度バッジ表示
- セクション2.5.4: スレッド専用質問機能の詳細を追加
- セクション7.1.5, 7.2.5: API設計の追加・更新
- セクション8.3: スレッド詳細画面のUI/UX設計を更新

### Version 1.1 (2025-11-04)
- ビュー保存機能の設計を追加（セクション13）
  - フィルタ条件をビューとして保存・管理
  - プルダウンからビューを選択して適用
  - デフォルトビュー設定機能
  - データモデル、API設計、UI/UX設計を含む包括的な設計

### Version 1.0 (2025-11-02)
- Phase 2 完了: Excelスタイルのフィルタリング機能実装
- Phase 3 完了: ChatGPT API要約機能実装
- Phase 4 完了: 新規スレッド発見機能実装
- Phase 5 完了: Claude Agent SDK質問応答機能実装（横断検索）
- UI/UX大幅改善: テーブルヘッダーフィルタ、ページネーション
- データモデル更新: ステータス管理、確信度計算

### Version 0.9 (2025-01-20)
- 初版作成
- Phase 1 基盤構築完了
